...
exports.getValue = (user, chatId, what) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/level.json'));
...
// ADICIONA VALORES
exports.addValue = (user, amount, chatId, what) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/level.json'));
	if (Object.keys(place).includes(chatId)) {
		if (Object.keys(place[chatId]).includes(user)) {
			...
			place[chatId][user] = {
				...
				stone: Number(what == 'stone' ? amount : 100),
				gold: Number(what == 'gold' ? amount : 100),
				iron: Number(what == 'iron' ? amount : 100),
				wood: Number(what == 'wood' ? amount : 100)
			};
		}
	} else {
		place[chatId] = {
...
exports.resetValue = (user, chatId) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/level.json'));
	if (Object.keys(place).includes(chatId)) {
		if (Object.keys(place[chatId]).includes(user)) {
			...
			place[chatId][user].stone = 100;
			place[chatId][user].gold = 100;
			place[chatId][user].iron = 100;
			place[chatId][user].wood = 100;
		} else {
			place[chatId] = {
				[user]: {
					xp: 0,
...
exports.wait = (userId) => {
	xpGain.add(userId);
	tools('others').sleep(Number(config.Wait_to_Win) * 1000).then(() => xpGain.delete(userId));
};

// Adiciona limite ao usuário
exports.addLimit = (user, chatId, what) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/limit.json'));
...
exports.getLimit = (user, chatId, getDetails, whoFilter) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/limit.json'));
	let Timings = {};
...
exports.getPatent = (nivel) => {
	let patents = JSON.parse(fs.readFileSync('./lib/config/Gerais/patentes.json'));
	var role = patents.a0;
	if (nivel <= 3) {
		role = role;
	} else if (nivel <= 5) {
...
// Adquire o XP necessário para upar
...
exports.changeGuild = (user, chatId, guild) => {
	let place = JSON.parse(fs.readFileSync('./lib/config/Gerais/level.json'));
